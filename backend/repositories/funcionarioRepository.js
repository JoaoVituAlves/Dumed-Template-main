...
export default class FuncionarioRepository extends BaseRepository {
    ...
    async validarAcesso(cpf, senha) {
        let sql = "SELECT * FROM tb_funcionarios WHERE cpf = ? AND senha = ?";
    ...
    async listar() {
        let sql = `
            ...
            INNER JOIN tb_cargo c ON f.id_cargo = c.id
            INNER JOIN tb_tipo t ON f.id_tipo = t.id 
            ORDER BY id;
        `;
    ...
    toMap(rows) {
        if (Array.isArray(rows)) {
            ...
            return new FuncionarioEntity(
                ...
                rows.cargo ?? null, rows.tipo ?? null
            );
        }
    }    
}
