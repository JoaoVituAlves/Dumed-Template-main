...
export default class ClienteController {
    ...
    async gravar(req, res) {
        try {
            ...
            let entidade = new ClienteEntity(
                0, nome, email, cpf, telefone, senha, tipo,
                cnpj, razao_social, insc_estadual, cep, rua, bairro, cidade, numero
            );
    
            let repo = new ClienteRepository();
            let jaExiste = await repo.buscarCpfCnpj(cpf, cnpj);
            if (jaExiste) {
    ...
    async alterar(req, res) {
        try {
            ...
            if (id && nome && email && cpf && telefone && tipo != null && cnpj && razao_social && insc_estadual && cep && rua && bairro && cidade && numero) {
                ...
                if(cpf != clienteAtual.cpf || cnpj != clienteAtual.cnpj) {
                    let jaExiste = await repo.buscarCpfCnpj(cpf, cnpj);
                    if (jaExiste) {
                        return res.status(400).json({ msg: "Já existe um cadastro com esse CPF ou CNPJ!" });
                    }
    ...
    async criarNovaSenha(req, res) {
        try {
            ...
            if (id && (senhaAtual || novaSenha)) {
                ...
                let result = await repo.criarNovaSenha(novaSenha, id);
                if (result) {
    ...
    async redefinirSenha(req, res) {
        ...
        try {
            if(token && novaSenha && confmNovaSenha) {
                ...
                await repo.alterarSenha(novaSenha, cliente.id);
