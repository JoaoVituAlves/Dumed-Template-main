    ...
    async gravar(req, res) {
        try {
            ...
            if (nome && cpf && telefone && email && data_admissao && id_cargo && status && id_tipo && senha) {
                ...
                let entidade = new FuncionarioEntity(0, nome, cpf, telefone, email, data_admissao, id_cargo, salario, status, id_tipo, senha);
                let repo = new FuncionarioRepository();
    ...
    async redefinirSenha(req, res) {
        ...
        try {
            if(token && novaSenha && confmNovaSenha) {
                ...
                await repo.alterarSenha(novaSenha, funcionario.id);
